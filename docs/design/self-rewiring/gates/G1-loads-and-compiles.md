...
| Added check | Why a proposer makes it necessary |
|---|---|
| **Every node reachable from `entry_node`** | A proposer can orphan a node by rewiring around it; an orphan is dead weight and usually a mistake |
| **`END` reachable** from `entry_node` | A rewiring that removes every path to `END` produces a graph that can only ever terminate via `max_steps` — a `GraphExecutionError`, discovered at runtime rather than at gate time |

Cycles are **not** rejected — cycles are legitimate (bounded by
`max_steps`, `executor.py:157`) and `test_max_steps_exceeded_raises`
...
3. Non-pure node with no `idempotency_key_fn` ⇒ reject
   (`NodeContractError`).
4. Manifest orphaning a node ⇒ reject (new reachability check).
5. Manifest with no path to `END` ⇒ reject (new END-reachability check).
...
- **Q-G1-1** — should reachability / END-reachability be promoted into
  `Graph.validate()` for all graphs, not just proposed ones? That would be
  a kernel behaviour change requiring its own ADR and could break existing
  hand-authored graphs. **Recommendation: keep in the gate for v1.**
