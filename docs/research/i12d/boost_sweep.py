"""N2 step 1a — the STATIC `knowledge_boost` sweep on the new seed.

Retrieval ranking only: no model, no live call. It answers half of the knob's
A/B — *where does the one lesson rank at each boost, on the seed as it stands,
against every validation objective* — and it is the half that decides the
value arm (c) runs at.
...
so the entry never goes stale. `dry_identity.py` runs the whole sequential arm
under a stub and gives the WORST case, in which nothing the stub answers ever
refreshes the lesson. The boost arm (c) runs at is chosen from the worst case,
so the arm engages whatever the live trajectory does.
...
BOOSTS = (0.0, 0.5, 1.0, 3.0, 4.0, 5.0, 6.0, 7.0, 8.0)
RENDER_MAX_ITEMS = 5
