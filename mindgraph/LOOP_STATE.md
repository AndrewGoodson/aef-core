...
- [ ] 0.1 — encoding contract as CODE: a validator that refuses to emit a visual property when its backing field is null, and forces the UNKNOWN construction (hatched interior, broken border, "?")
...
NEXT: 0.1 — write the encoding contract as code, with the null-field → UNKNOWN construction enforced at build time
BLOCKERS: none
MANUAL_CHECKS:
...
LAST_RUN: 2026-08-05 — initialization. Scaffolded dirs, installed spec, built fixtures covering all 11 required edge cases, wrote tools/verify. Verified: self-test PASS (14/14 planted faults detected, no false positive on clean input); fixture checks PASS (12/12); dist/index.html check FAIL — expected, nothing is built until Stage 2, and the verifier is deliberately left red rather than stage-gated.
