...
NEXT: 1.2 — offline layout with deterministic seeding only (weighted centroid of positioned neighbours; perimeter by hash of stable ID; never Math.random)
BLOCKERS: none
MANUAL_CHECKS:
...
LAST_RUN: 2026-08-05 — increment 1.1. pipeline/traversal.py derives a directly-follows graph from THREE inputs, not one: the event log (what happened), the declared topology (what is configured), and the coverage declaration (whether we could have seen it). Deriving from the log alone would make an edge nobody has taken indistinguishable from an edge that does not exist. `retired` comes only from an explicit retired_at — dead is never inferred from inactivity. `unknown` coverage beats everything and makes NO zero-count claim. Added fixtures/topology.json and fixtures/events.jsonl (162 runs, 663 events, fully deterministic). The derivation caught two defects in my own fixtures: an observed traversal with no declared edge, and a never_observed state that had no instance because the log actually took that path. Both fixed in the fixtures, not by loosening the classifier. Verified: self-test PASS (19 detections); verify 40/41 PASS including all five states derived, determinism, and dormant-vs-never-observed separated in DERIVED output rather than asserted in a hand-written fixture; dist/index.html FAIL — expected until Stage 2.
